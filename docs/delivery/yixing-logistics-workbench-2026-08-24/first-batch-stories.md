...
Status: `DEFINED_BUT_GATE_BLOCKED`.

| Order | Story/work | Existing tests | Entry condition |
| --- | --- | --- | --- |
| 1 | G7 capability and contract evidence | Contract suite to link to existing Story tests | Rotated SANDBOX Secret references and target app permissions |
...
No other Epic starts concurrently. `US-069..074` are separately blocked because their Story definitions contain no `TEST-*` IDs; the source's Build Ready statement also still says 68 FR although the PRD contains 82.
