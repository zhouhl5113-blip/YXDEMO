...
| HAR-002 | Runtime and connectors (partially resolved) | The initiator selected the current Windows Server 2025 host for local development/testing. WSL 1 + Ubuntu 24.04 is pending restart; this may back `yixing_dev`/`yixing_preprod` after validation. Production remains unresolved because the single host has no independent failure domain, HA or recovery evidence. Source, registry, deployment and observability connector refs are still required | Design / Platform owner |
